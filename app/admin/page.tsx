...
export default function AdminPage() {
  ...
  async function save(slug: Row["slug"], blurb: string) {
    ...
    try {
      ...
      setRows((rs) =>
        rs.map((r) => (r.slug === slug ? { ...r, saving: false, saved: true } : r))
      );

      // brief success tick
      setTimeout(() => {
        setRows((rs) =>
          rs.map((r) => (r.slug === slug ? { ...r, saved: false } : r))
        );
      }, 1200);
    } catch (err: unknown) {
      const msg =
      ...
      setRows((rs) =>
        rs.map((r) => (r.slug === slug ? { ...r, saving: false, error: msg } : r))
      );
    }
  }
    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-100">
      <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        <p className="mt-2 text-neutral-400 text-sm">
          Protected by Basic Auth. Edit product blurbs and click <span className="text-neutral-200">Save</span>.
        </p>

        <div className="mt-8 space-y-6">
          {rows.map((r) => (
            <div key={r.slug} className="rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setRows((rs) =>
                    rs.map((x) => (x.slug === r.slug ? { ...x, blurb: e.target.value } : x))
                  )
                }
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                {r.saved && <span className="text-emerald-300 text-sm">Saved ✓</span>}
                {r.error && <span className="text-rose-300 text-sm">{r.error}</span>}
              </div>
            </div>
          ))}
        </div>
